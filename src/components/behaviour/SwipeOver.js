...
export const swipeTopDirection = (direction, reverse) => {
...
export const swipeBottomDirection = (
  direction,
  reverse = false,
  offset = 40
) => {
...
export const swipe = ({
  ...
  if (triggerName === "entry" && top === "entry") {
    return gsap
      ...
      .from(node, {
        ...swipeTopDirection(direction, true),
        duration: exit.length,
      })
      .set(node, { overflowY: "initial" })
  } else if (triggerName === "entry") {
    return gsap.timeline().from(node, {
  ...
  return (
    <TransitionLink
      ...
      exit={{
        length: exitLength,
        trigger: ({ node, exit }) => {
          const customProps = onExit(node, exit)
          swipe({
            node,
            exit,
      ...
      entry={{
        ...
        delay: 1,
        trigger: ({ node, exit }) => {
          const customProps = onEntry(node, exit)
          swipe({
            node,
            exit,
            direction: props.direction,
